...
use crate::{
    ...
    utils::back_reader_unsafe::UnsafeBackReader,
};

pub fn trim_redstone_marker(payload: &mut UnsafeBackReader) -> Result<(), Error> {
    let marker = payload.unsafe_read_vec(REDSTONE_MARKER_BS);

    if marker.as_slice() != &REDSTONE_MARKER {
        return Err(Error::WrongRedStoneMarker(marker.deref().clone()));
    }
