...
impl<'a, C: Crypto> PayloadDecoder<'a, C> {
    ...
    fn trim_data_package(&mut self, payload: &mut Vec<u8>) -> Result<DataPackage, Error> {
        ...
        let meta_size = DATA_POINTS_COUNT_BS + DATA_POINT_VALUE_BYTE_SIZE_BS + TIMESTAMP_BS;

        let mut meta = payload[payload.len().saturating_sub(meta_size)..payload.len()].to_vec();

        let data_point_count = meta.try_trim_end(DATA_POINTS_COUNT_BS)?;
        let value_size = meta.try_trim_end(DATA_POINT_VALUE_BYTE_SIZE_BS)?;
        let timestamp = meta.try_trim_end(TIMESTAMP_BS)?;

        let size = (TryInto::<usize>::try_into(data_point_count)?)
            * (TryInto::<usize>::try_into(value_size)? + DATA_FEED_ID_BS)
            + DATA_POINT_VALUE_BYTE_SIZE_BS
            + TIMESTAMP_BS
            + DATA_POINTS_COUNT_BS;

        let signable_bytes: Vec<_> =
            payload[payload.len().saturating_sub(size)..payload.len()].to_vec();

        let signer_address = self.crypto.recover_address(signable_bytes, signature).ok();

        let _: Vec<_> = payload.trim_end(meta_size);
