use alloc::vec::Vec;
use core::{convert::TryInto, ops::Deref, ptr::read};
...
use crate::{
    ...
    utils::{back_reader_unsafe::UnsafeBackReader, trim::Trim},
    TimestampMillis,
};
...
impl<'a, C: Crypto> PayloadDecoder<'a, C> {
    ...
    pub fn make_payload(&mut self, mut payload_bytes: Vec<u8>) -> Result<Payload, Error> {
        let mut unsafe_back_reader = UnsafeBackReader::new(&mut payload_bytes);

        trim_redstone_marker(&mut unsafe_back_reader)?;

        let payload = self.trim_payload(&mut unsafe_back_reader)?;

        if !unsafe_back_reader.is_empty() {
            return Err(Error::NonEmptyPayloadRemainder(unsafe_back_reader.len()));
        }

        Ok(payload)
    }

    fn trim_payload(&mut self, reader: &mut UnsafeBackReader) -> Result<Payload, Error> {
    ...
    fn trim_metadata(&self, reader: &mut UnsafeBackReader) -> Result<usize, Error> {
        let unsigned_metadata_size = reader
            .unsafe_read_u64(UNSIGNED_METADATA_BYTE_SIZE_BS)?
            .try_into()?;

        reader.unsafe_read_vec(unsigned_metadata_size);

        let data_package_count = reader.unsafe_read_u64(DATA_PACKAGES_COUNT_BS)?.try_into()?;
    ...
    fn trim_data_package(&mut self, reader: &mut UnsafeBackReader) -> Result<DataPackage, Error> {
        let signature = reader.unsafe_read_vec(SIGNATURE_BS);

        let data_point_count = reader.unsafe_read_u64(DATA_POINTS_COUNT_BS)?;
        let value_size = reader.unsafe_read_u64(DATA_POINT_VALUE_BYTE_SIZE_BS)?;
        let timestamp = reader.unsafe_read_u64(TIMESTAMP_BS)?;

        let data_points_size =
            data_point_count * (value_size + TryInto::<u64>::try_into(DATA_FEED_ID_BS)?);

        let size: u64 = data_points_size
        ...
        reader.move_pointer(DATA_POINTS_COUNT_BS + TIMESTAMP_BS + DATA_POINT_VALUE_BYTE_SIZE_BS);
        let mut signable_bytes = reader.unsafe_read_vec(size.try_into()?);

        let signer_address = self
            .crypto
            .recover_address(signable_bytes.as_slice(), signature.as_slice())
            .ok();

        reader.move_pointer(data_points_size.try_into()?);
    ...
    fn trim_data_point(reader: &mut UnsafeBackReader, value_size: usize) -> DataPoint {
        let value = reader.unsafe_read_vec(value_size);
        let feed_id = reader.unsafe_read_vec(DATA_FEED_ID_BS);

        DataPoint {
            value: value.deref().clone().into(),
            feed_id: feed_id.deref().clone().into(),
        }
    }
